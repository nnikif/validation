...
$(document).ready(function() {
  MyForm.initialize($("#myForm"),$("#resultContainer"))
...
var MyForm = {
  self:this,
  initialize: function(input_form,output_form){
    this.node=input_form;
    this.output_node=output_form;
  },
  validate: function () {
  ...
  getData: function () {
    ...
    var returned={};
    console.log(document.myForm.fio);
    $.each(validators, function(field) { returned[field] = document.myForm[field];});
    return returned;
  ...
  setData: function(form_values){
    ...
    $.each(validators, function (field) {
      if (form_values[field]) {self.node.find("input[name=" + field + "]").val(form_values[field]);}
